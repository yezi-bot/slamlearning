...
int main ( int argc, char** argv )
{
    ...
    auto graph = make_shared<gtsam::NolinearFactorGraphy>(); // gtsam的因子图
    auto initial = make_shared<gtsam::Values>() // 初始值
    ...
    while ( !fin.eof() )
    {
        ...
        else if ( tag == "EDGE_SE3:QUAT" )
        {
            ...
            auto factor =make_shared<gtsam::NonlinearFactor>( 
                 gtsam::BetweenFactor<gtsam::Pose3> ( id1, id2, gtsam::Pose3 ( R,t ), model ) 
            );
            graph->push_back ( factor ); // 添加一个因子
            cntEdge++;
        }
        if ( !fin.good() )
            break;
    ...
    //对角式高斯噪声模型
    auto priorModel = make_shared<gtsam::noiseModel::Diagonal>(
        gtsam::noiseModel::Diagonal::Variances (
            ( gtsam::Vector ( 6 ) <<1e-6, 1e-6, 1e-6, 1e-6, 1e-6, 1e-6 ).finished() //入参是6个方向的方差
        ));
