...
void BuildProblem(const BALProblem* bal_problem, g2o::SparseOptimizer* optimizer, const BundleParams& params)
{
    ...
    const double* raw_cameras = bal_problem->cameras(); //cam0,cam1,cam2(9)
    for(int i = 0; i < num_cameras; ++i)  //历遍所有相机
    {
        ConstVectorRef temVecCamera(raw_cameras + camera_block_size * i,camera_block_size);//给9块套一个向量
    ...
    for(int j = 0; j < num_points; ++j)
    {
        ...
        pPoint->setId(j + num_cameras);   //保证所有编号不重复 
        pPoint->setMarginalized(true);   //使用Schur
...
void WriteToBALProblem(BALProblem* bal_problem, g2o::SparseOptimizer* optimizer) //原始数据，稀疏优化器
{
    const int num_points = bal_problem->num_points();
    const int num_cameras = bal_problem->num_cameras();
    const int camera_block_size = bal_problem->camera_block_size();
    ...
    for(int i = 0; i < num_cameras; ++i)
    {
        ...
        Eigen::VectorXd NewCameraVec = pCamera->estimate();  //获得9维相机参数
...
{
    g2o::OptimizationAlgorithmWithHessian* solver;   //父类
...
void SolveProblem(const char* filename, const BundleParams& params)
{
    ...
    srand(params.random_seed);    //设置随机数种子
    bal_problem.Normalize();          //归一化全部三维坐标，把点云挪到原点
    ...
    SetSolverOptionsFromFlags(&bal_problem, params, &optimizer);  //创建所有相机顶点、路标点顶点、重投影误差边
