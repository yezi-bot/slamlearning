...
void VisualOdometry::featureMatching()
{
    ...
    cv::BFMatcher matcher ( cv::NORM_HAMMING );      //汉明法暴力匹配
    ...
    {
        return m1.distance < m2.distance;  //匹配规则
    } )->distance;
...
void VisualOdometry::setRef3DPoints()
{
    ...
    for ( size_t i=0; i<keypoints_curr_.size(); i++ )
    {
        ...
        if ( d > 0)
        {
            ...
            descriptors_ref_.push_back(descriptors_curr_.row(i));       //每一行是一个关键点
        }
    }
}
...
void VisualOdometry::poseEstimationPnP()
{
    ...
    {
        pts3d.push_back( pts_3d_ref_[m.queryIdx] );  //参考帧特征点[x,y,z]
        pts2d.push_back( keypoints_curr_[m.trainIdx].pt );  //新图片像素位置[x,y]
    }
...
bool VisualOdometry::checkEstimatedPose()
{
    ...
    Sophus::Vector6d d = T_c_r_estimated_.log();         //对SE3取对数得到李代数
    if ( d.norm() > 5.0 )        //模长
    {
